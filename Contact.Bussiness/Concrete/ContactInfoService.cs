...
namespace Contact.Bussiness.Concrete
{
    public class ContactInfoService : IContactInfoService
    {
        ...
        public IDataResult<ContactInfo> Add(ContactInfo contactInfo)
        {
            ...
            if(contactInfo.InfoType.Equals(InfoType.Location))
            {
                var locations = _contactInfoDal.GetList(w => w.PersonUUID.Equals(person.UUID) && w.InfoType.Equals(InfoType.Location));
                if (locations.Count() > 0)
                    return new ErrorDataResult<ContactInfo>(locations.First(), "A location info already exists");
            }
